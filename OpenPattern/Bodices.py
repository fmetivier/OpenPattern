...
class Basic_Bodice(Pattern):
    ...
    def Donnanno_bodice_without_dart_w(self, bust_ease=8):
        ...
        # Frame
        #################################################
        A = Point([0, self.m["longueur_devant"]])
        B = Point([0, 0])
        C = Point([(self.m["tour_poitrine"] + bust_ease) / 2, 0])
        D = Point([(self.m["tour_poitrine"] + bust_ease) / 2, self.m["longueur_dos"]])
        E = Point([(self.m["tour_poitrine"] + bust_ease) / 4, 0])
        F = Point(
            [(self.m["tour_poitrine"] + bust_ease) / 4, self.m["longueur_devant"]]
        )
        ...
        #################################################
        H = Point(
            [(self.m["tour_poitrine"] + bust_ease) / 2, self.m["longueur_dos"] / 2]
        )
        I = Point([0, self.m["longueur_dos"] / 2])
        Q = Point([E.x, H.y])

        # G = Point([3*(self.m["tour_poitrine"]+bust_ease)/10, self.m["longueur_dos"]])
        # H1 = Point([3*(self.m["tour_poitrine"]+bust_ease)/10, self.m["longueur_dos"]/2])
        ...
        if self.m["longueur_epaule"] == 0:
            ...
            print("[PO]", dPO)
            dOP1 = self.m["longueur_epaule"] - dPO
        ...
        if self.m["longueur_epaule"] == 0:
            ...
            Z2 = Point(
                [
                    U.x + self.m["longueur_epaule"] * np.cos(a),
                    U.y + self.m["longueur_epaule"] * np.sin(a),
                ]
            )
    ...
    def Donnanno_bodice_without_dart_m(self, bust_ease=24):
        ...
        G = D - [
            self.m["largeur_epaule"] / 2 + 1,
            0,
        ]  # ca merde dans les "grandes" largeurs... j'utilise donc la méthode pour les femmes
        # ~ G = D - [(m["tour_poitrine"]+bust_ease)/5 +1, 0]
        H1 = H - [(self.m["largeur_epaule"] / 2 + 1), 0]
        # ~ H1 = H - [(m["tour_poitrine"]+bust_ease)/5 +1, 0]

        # 1/5 of HI does not work -> try 1/10
        I1 = H1 - [(self.m["tour_poitrine"] + bust_ease) / 10 + 2, 0]
        J1 = Point([I1.x, A.y])
        ...
        if self.m["longueur_epaule"] == 0:
            ...
            dPO = self.distance(P, O)
            print("[PO]", dPO)
            dOP1 = self.m["longueur_epaule"] - dPO + 1
        ...
        if self.m["longueur_epaule"] == 0:
            ...
            Z2 = U + [
                (self.m["longueur_epaule"] + 1) * np.cos(a),
                (self.m["longueur_epaule"] + 1) * np.sin(a),
            ]
            print("[UZ2]", self.distance(U, Z2))
        ...
        Y = B - [0, self.m["hauteur_bassin"]]
        X = C - [0, self.m["hauteur_bassin"]]
        E1 = self.middle(X, Y)
        ...
        l_base_front, base_front = self.pistolet(
            np.array([E1, lE1, rB12, rB11, rB10]), 3, tot=True
        )
    ...
    def chiappetta_basic_bodice(self, age=14, d_FB=5):
        ...
        if age >= 2 and age <= 16:
            ...
            ShB1 = CB2 + [
                self.m["longueur_epaule"] * np.cos(np.pi * (1 + sa / 180)),
            ...
            ClCF = Point(
                [
                    CF2.x + lcf * np.cos(3 * np.pi / 4),
                    CF1.y + lcf * np.sin(3 * np.pi / 4),
                ]
            )

            collar_front_points = np.array([CF1, ClCF, CF2])
            self.m["longueur_col_devant"], self.front_collar_curve = self.pistolet(
                collar_front_points, 2, tot=True
            )

            ShF1 = CF2 + [
                self.m["longueur_epaule"] * np.cos(-25 * np.pi / 180),
            ...
            sleeve_front_points = [ShF1, BF1, SlF1]
            (
    ...
    def chiappetta_basic_bodice__ori_m(self, BF_space=2):
        ...
        P13 = P10 + Point(
            [
                -(1 / 10) * self.m["tour_encolure"] * np.cos(np.pi / 4),
                +(1 / 10) * self.m["tour_encolure"] * np.sin(np.pi / 4),
            ]
        )
        ...
        P12 = P11 + Point(
            [
                self.m["longueur_epaule"] * np.cos(a),
                -self.m["longueur_epaule"] * np.sin(a),
            ]
        )
        ...
        I = H + Point(
            [
                np.cos(np.pi / 4) * self.m["tour_encolure"] / 20,
                np.sin(np.pi / 4) * self.m["tour_encolure"] / 20,
            ]
        )
        ...
        K = J - Point(
            [
                np.cos(a) * (self.m["longueur_epaule"] + 1),
                np.sin(a) * (self.m["longueur_epaule"] + 1),
            ]
        )

        # courbes
        # encolure
        self.m["longueur_col_devant"], front_collar_curve = self.pistolet(
            [P8, P13, P11], 2, tot=True
        )
        self.m["longueur_col_dos"], back_collar_curve = self.pistolet(
            [B, I, J], 2, tot=True
        )
        ...
        P7b = P7 + Point([0, -2])
        self.m["longueur_emmanchure_dos"], back_sleeve_curve = self.pistolet(
            [K, K1, G, G1, E], 3, tot=True
        )
        print("emmanchure dos", self.m["longueur_emmanchure_dos"])
        self.m["longueur_emmanchure_devant"], front_sleeve_curve = self.pistolet(
            [P12, P12b, P7, P7b, P6], 3, tot=True
        )
    ...
    def chiappetta_basic_bodice_m(self):
        ...
        ShB1 = CB2 - Point(
            [
                np.cos(b) * (self.m["longueur_epaule"] + 1),
                np.sin(b) * (self.m["longueur_epaule"] + 1),
            ]
        )
        ...
        # encolure
        self.m["longueur_col_devant"], self.front_collar_curve = self.pistolet(
            [CF, ClCF, CF2], 2, tot=True
        )
        self.m["longueur_col_dos"], self.back_collar_curve = self.pistolet(
            [HB, ClCB, CB2], 2, tot=True
        )
    ...
    def chiappetta_armhole_sleeve_m(
        ...
        for i in np.arange(len(fc) - 1):
            d += np.sqrt(
                (fc[N - i][0] - fc[N - i - 1][0]) ** 2
                + (fc[N - i][1] - fc[N - i - 1][1]) ** 2
            )
            if d <= 6:
                fc2.append(fc[N - i])
        ...
        if folds == 1:
            ...
            F = C + Point(
                [self.m["tour_poignet"] / 2 + ease / 2 + folds * fold_width, 0]
            )
        elif folds == 2:
            G = C - Point([self.m["tour_poignet"] / 2 + ease / 2, 0])
            F = C + Point(
            ...
            F = C + Point(
                [self.m["tour_poignet"] / 2 + ease / 2 + (folds - 1) * fold_width, 0]
            )
        ...
        for i in np.arange(len(end_front_curve) - 1):
            dfront += np.sqrt(
                (end_front_curve[i + 1][0] - end_front_curve[i][0]) ** 2
                + (end_front_curve[i + 1][1] - end_front_curve[i][1]) ** 2
            )
        dback = 0
        for i in np.arange(len(end_back_curve) - 1):
            dback += np.sqrt(
                (end_back_curve[i + 1][0] - end_back_curve[i][0]) ** 2
        ...
        print(
            "longueur totale",
            dfront + darm + dback,
            self.m["longueur_emmanchure_dos"] + self.m["longueur_emmanchure_devant"],
        )
    ...
    def chiappetta_basic_sleeve_m(self):
        ...
        B = A - Point([0, (4.0 / 5) * self.m["profondeur_emmanchure"] - 1])

        E = B - Point([(3.0 / 4) * self.m["longueur_emmanchure_devant"], 0])
        D = B + Point([(3.0 / 4) * self.m["longueur_emmanchure_dos"], 0])
    ...
    def Gilewska_basic_bodice_m(self, BF_space=10):
        ...
        CB2 = CB1
        CB2b = HB + [1, 0]
        self.m["longueur_col_dos"], collar_back_points = self.pistolet(
            np.array([HB, CB2b, CB1]), 2, tot=True
        )
        ...
        CPSlB = Point(
            [
                BB1.x + np.cos(np.pi / 4) * b_length,
                SlB1.y + np.sin(np.pi / 4) * b_length,
            ]
        )
        # ~ CPSlB1 = SlB1 - [0.5, 0]
        ...
        ShF1 = Point(
            [HF1.x, HF1.y - 7]
        )  # valeur Gilewska je la trouve très élevée et les longueurs d'épaules ne coincident pas
        ...
        if self.m["longueur_epaule"] > 0:
            ...
            print("delta", delta)
            ShF1 = ShF1 + [
                delta * np.cos(a + np.pi),
                delta * np.sin(a + np.pi),
            ]  # add length difference

            dShF = self.distance(CF1, ShF1)  # recalculate and check
        print(
            "longueur épaule devant\n\t mesurée:%4.0f\n\t calculée: %4.0f"
            % (self.m["longueur_epaule"], dShF)
        )
        ...
        CPSlF = Point(
            [
                BF1.x - np.cos(np.pi / 4) * f_length,
                SlF1.y + np.sin(np.pi / 4) * f_length,
            ]
        )
        ...
        if self.hip:
            ...
            self.Back_vertices = (
                [HiB.pos(), WB.pos(), HB.pos()]
                + collar_back_points
                + sleeve_back_points
                + [SlB1.pos(), HiB1.pos()]
            )
            self.Front_vertices = (
                [HiF.pos(), SlF.pos(), BF.pos()]
                + collar_front_points
                + sleeve_front_points
                + [SlF1.pos(), HiF1.pos()]
            )
        ...
        self.curves_dic = {
            "Back_Collar": collar_back_points,
            "Back_Sleeve": sleeve_back_points,
            "Front_Collar": collar_front_points,
            "Front_Sleeve": sleeve_front_points,
        }
    ...
    def Gilewska_basic_sleeve_m(self):
        ...
        E = A + [
            -1.5,
            0,
        ]  # There's an uncertainty in the  book because on fig 3 p 81 it's 1cm but in the text it's 1.5
        F = A + [0.5, 0]
        ...
        print(a)
        Gb1 = Point(
            [Gb.x + 0.8 * np.cos(a - np.pi / 2), Gb.y + 0.8 * np.sin(a - np.pi / 2)]
        )
        Gh1 = Point(
            [Gh.x + 1.8 * np.cos(a + np.pi / 2), Gh.y + 1.8 * np.sin(a + np.pi / 2)]
        )
    ...
    def Gilewska_basic_bodice_w(self, sep=10):
        ...
        CPCB = Point(
            [
                self.m["largeur_encolure"] - 1.5 * np.cos(np.pi / 4),
                self.m["longueur_dos"]
                - self.m["profondeur_encolure_dos"]
                + 1.5 * np.sin(np.pi / 4),
            ]
        )
        CPCF = HF + [
            -self.m["largeur_encolure"] + 2.5 * np.cos(np.pi / 4),
            -self.m["profondeur_encolure_devant"] + 2.5 * np.sin(np.pi / 4),
        ]
        ...
        self.m["hauteur_carrure"] = (
            self.m["longueur_dos"]
            - self.m["hauteur_emmanchure"]
            - self.m["profondeur_encolure_dos"]
        ) / 3 + 1
        ...
        ShB1 = Point(
            [
                self.m["largeur_encolure"] + x_epaule_dos,
                self.m["longueur_dos"] - y_epaule_dos,
            ]
        )
        ShF1 = Point(
            [
                HF.x - self.m["largeur_encolure"] - x_epaule_devant,
                self.m["longueur_devant"] - y_epaule_devant,
            ]
        )
        ...
        CPSlB = Point(
            [
                self.m["carrure_dos"] / 2 + b_length * np.cos(np.pi / 4),
                self.m["hauteur_emmanchure"] + b_length * np.sin(np.pi / 4),
            ]
        )
        ...
        CPSlF = Point(
            [
                SlF.x - self.m["carrure_devant"] / 2 - f_length * np.cos(np.pi / 4),
                self.m["hauteur_emmanchure"] + f_length * np.sin(np.pi / 4),
            ]
        )
    ...
    def add_bust_dart(self):
        ...
        if self.style in ("Gilewska", "Chiappetta"):
            ...
            self.curves_dic[
                "Front_Sleeve"
            ] = new_sleeve_front_points  # I change the armhole curve to the new one.
            self.m["longueur_emmanchure_devant"] = dsf
    ...
    def Gilewska_basic_sleeve_w(self):
        ...
        C = Point(
            [0, self.m["longueur_manche"] - 4 * self.m["profondeur_emmanchure"] / 5]
        )
        ...
        CDos4 = KK + [
            0.5 * np.cos(self.segment_angle(D, K) - np.pi / 2),
            0.5 * np.sin(self.segment_angle(D, K) - np.pi / 2),
        ]
        CDevant4 = KK1 + [
            0.8 * np.cos(self.segment_angle(E, K1) - np.pi / 2),
            0.8 * np.sin(self.segment_angle(E, K1) - np.pi / 2),
        ]
