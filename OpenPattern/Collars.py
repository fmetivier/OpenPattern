...
class Collars(Pattern):
    ...
    def calculate_collar_gilewska_m(self, overlap=0, collar_height=3):
        ...
        if self.Collar_style == "OnePiece":
            ...
            self.Collar_vertices.append(
                curve_dic["bcd"] + curve_dic["fcd"] + curve_dic["fcc"] + upper_curve
            )
        ...
        if self.Collar_style == "TwoPieces":
            ...
            curve_dic = self.pied_de_col_gilewska_m(
                overlap, collar_height, front_height=1.5
            )
            ...
            self.Collar_vertices.append(
                TC_down_curve + [TClu.pos(), TCru.pos(), TCrd.pos()]
            )
            self.Collar_dic.append(
                {"TCrd": TCrd, "TCru": TCru, "TClu": TClu, "TCld": TCld}
            )

            # redefine the size of segments
            A = Point(
                [self.m["longueur_col_dos"] + self.m["longueur_col_devant"] + 1, 0]
            )
            Cmd = self.Collar_dic[0]["Cmd"]
            C7d = self.Collar_dic[0]["C7d"]
            self.Collar_segments = {
                "Middle Back": [Point([0, 0]), Point([0, 15])],
                "Shoulder": [Cmd, Cmd + [0, 15]],
                "Middle Front": [A, A + [0, 15]],
                "Overlap Line": [C7, C7d],
            }
    ...
    def pied_de_col_gilewska_m(self, overlap=0, collar_height=3, front_height=2):
        ...
        C3 = Cmd + 0.5 * dcf * np.array(
            [np.cos(a - 2 * np.pi / 180), np.sin(a - 2 * np.pi / 180)]
        )
        ...
        Cru = Crd + [
            collar_height * np.cos(a + np.pi / 2),
            collar_height * np.sin(a + np.pi / 2),
        ]
        if collar_height > 5:
        ...
        if overlap == 0:
            ...
            C7 = C7d + [
                np.cos(ad + np.pi / 2) * collar_height,
                np.sin(ad + np.pi / 2) * collar_height,
            ]
        ...
        C8 = Cmu + 0.5 * self.distance(Cmu, C6) * np.array(
            [np.cos(a - 2 * np.pi / 180), np.sin(a - 2 * np.pi / 180)]
        )
        ...
        self.Collar_segments = {
            "Middle Back": [Point([0, 0]), Point([0, 10])],
            "Shoulder": [Cmd, Cmd + [0, 10]],
            "Middle Front": [A, A + [0, 10]],
        }
    ...
    def draw_collar(self, save=False, paper="FullSize"):
        ...
        fig, ax = self.draw_pattern(
            self.Collar_dic, self.Collar_vertices, self.Collar_polylines
        )
        ...
        for key, val in self.Collar_segments.items():
            ...
            ax.text(
                lbl_pos.x + 0.2 * np.cos(a + np.pi / 2),
                val[1].y,
                key,
                rotation=angle,
                ha="center",
