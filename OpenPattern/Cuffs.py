...
class Cuffs(Pattern):
    ...
    def calculate_cuffs_gilewska_m(self, overlap=2, width=5, ease=3):
        ...
        if self.Cuff_style == "Simple":
            ...
            self.Cuffs_dic.append(
                {"Cld": Cld, "Clu": Clu, "Olu": Olu, "Oru": Oru, "Ord": Ord, "Old": Old}
            )
            self.Cuffs_vertices.append(
                [Cld.pos(), Clu.pos(), Oru.pos(), Ord.pos(), Cld.pos()]
            )
            ...
            self.Cuffs_vertices.append(
                [Cld.pos(), Clu.pos(), Cru.pos(), Crd.pos(), Cld.pos()]
            )
            self.Cuffs_segments = {
                "Cuff_Fold": [Clm, Crm],
                "L_Overlap": [Olu, Old],
