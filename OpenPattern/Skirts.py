...
class Basic_Skirt(Pattern):
    ...
    def Gilewska_basic_skirt(self):
        ...
        H3 = C + Point(
            [0, -14]
        )  # place a control point slightly below the depth of the back dart
    ...
    def chiappetta_basic_skirt(self):
        ...
        dbskirt_f, self.skirt_front_side = self.pistolet(
            points_skirt_front, 2, tot=True
        )
        ...
        if self.pname not in ["W14C", "W16C"]:
            ...
            I3, I4 = self.add_dart(
                dart2, B2, H, dw, draw_curves=front_curves, order="rl"
            )
        else:
    ...
    def donnanno_basic_skirt(self):
        ...
        if self.curves:
            ...
            self.Back_vertices = [
                [F.pos(), C.pos(), D1.pos(), T3.pos(), S3.pos(), T2.pos(), W.pos()]
                + self.skirt_back_side[::-1]
                + [F.pos(), C.pos()]
            ]
            self.Front_vertices = [
                [B.pos(), A1.pos(), T4.pos(), S4.pos(), T5.pos(), W1.pos()]
                + self.skirt_front_side
                + [E2.pos(), F.pos(), B.pos()]
            ]
...
class Waistband(Pattern):
    ...
    def __init__(self, pname="W6C", ease=8, height=5, style="Donnanno", **kwargs):
        """
        Initilizes parent class &  attributes
...
class Skirt_transform(Basic_Skirt):
    ...
    def __init__(
        ...
        **kwargs
        ...
        Basic_Skirt.__init__(self, pname, style, gender, ease, curves, **kwargs)
    ...
    def shifted_side_seams(self, offset=2, pleat_height=20, pleat_width=8):
        ...
        if self.curves:
            ...
            self.Back_vertices = [
                [
                    self.Back_dic["C"].pos(),
                    self.Back_dic["F"].pos(),
                    self.Back_dic["E1"].pos(),
                ]
                + self.skirt_back_side
                + [self.Back_dic["W"].pos()]
                + T2
                + [self.Back_dic["S3"].pos()]
                + T3
            ...
            self.Front_vertices = [
                [self.Front_dic["B"].pos(), self.Front_dic["A1"].pos()]
                + T4
                + [self.Front_dic["S4"].pos()]
                + T5
                + [self.Front_dic["W1"].pos()]
                + self.skirt_front_side
                + [
                    self.Front_dic["E2"].pos(),
                    self.Front_dic["F"].pos(),
                    self.Front_dic["B"].pos(),
                ]
            ]
            ...
            self.Back_vertices = [
                [
                    self.Back_dic["F"].pos(),
                    self.Back_dic["C"].pos(),
                    self.Back_dic["D1"].pos(),
                    self.Back_dic["T3"].pos(),
                    self.Back_dic["S3"].pos(),
                    self.Back_dic["T2"].pos(),
                    self.Back_dic["W"].pos(),
                ]
                + self.skirt_back_side[::-1]
                + [self.Back_dic["F"].pos(), self.Back_dic["C"].pos()],
                [self.Back_dic["C"].pos(), C1.pos(), X1.pos(), X.pos()],
            ]
            ...
            self.Front_vertices = [
                [
                    self.Front_dic["B"].pos(),
                    self.Front_dic["A1"].pos(),
                    self.Front_dic["T4"].pos(),
                    self.Front_dic["S4"].pos(),
                    self.Front_dic["T5"].pos(),
                    self.Front_dic["W1"].pos(),
                ]
                + self.skirt_front_side
                + [
        ...
        self.add_labelled_line(
            self.middle(X, X1), self.middle(self.Back_dic["C"], C1), "FOLD", "l"
        )
    ...
    def A_Line(self, side_offset=5):
        ...
        dbskirt_b, self.skirt_front_side = self.pistolet(
            points_skirt_front, 2, tot=True
        )
        ...
        del self.fold_line[1]
        self.set_fold_line(
            self.Back_dic["H"] + Point([0, -2]),
            self.Back_dic["C"] + Point([0, 2]),
            "right",
        )
    ...
    def Flared_A_Line(self, side_offset=5):
        ...
        # cutting points
        HR = Point(
            [self.Front_dic["S4"].x, 0]
        )  # vertical projection of S4 on hem = cutting point
        self.Front_dic["HR"] = HR

        HR = Point(
            [self.Back_dic["S3"].x, 0]
        )  # vertical projection of S3 on hem = cutting point
        self.Back_dic["HR"] = HR
        ...
        if self.curves:
            ...
            # self.waist_curves = [T2,T3,T4,T5]
            num_f = self.distance(
                Point(self.waist_curves[2][-1]), Point(self.waist_curves[3][0])
            )
            self.Front_dic["S4"].y = self.Back_dic[
                "S3"
            ].y  # lower S4 to the level of S3 so the rotation is the same
            ...
            T2.move(
                [6 * side_offset, 0]
            )  # offset times 2  because we start from the A-line skirt.
...
class Culotte(Basic_Skirt):
    ...
    def __init__(
        ...
        **kwargs
        ...
        Basic_Skirt.__init__(self, pname, style, gender, ease, curves=False, **kwargs)
    ...
    def donnanno_basic_culotte(self):
        ...
        self.Front_dic["T4"], self.Front_dic["T5"] = self.add_dart(
            self.Front_dic["S4"], self.Front_dic["A1"], self.Front_dic["W1"], dwfb
        )
        self.Back_dic["T2"], self.Back_dic["T3"] = self.add_dart(
            self.Back_dic["S3"], self.Back_dic["D1"], self.Back_dic["W"], dwfb
        )
        ...
        self.Back_vertices = [
            ...
            + self.skirt_back_side[::-1]
            + [self.Back_dic["F"].pos(), self.Back_dic["C1"].pos()]
        ]
        self.Front_vertices = [
            [self.Front_dic["B1"].pos(), self.Front_dic["I"].pos()]
            + front_fourche
            + [
                self.Front_dic["A1"].pos(),
                self.Front_dic["T4"].pos(),
        ...
        #################################################
        front_waist = self.distance(
            self.Front_dic["A1"], self.Front_dic["T4"]
        ) + self.distance(self.Front_dic["T5"], self.Front_dic["W1"])
        back_waist = self.distance(
            self.Back_dic["W"], self.Back_dic["T2"]
