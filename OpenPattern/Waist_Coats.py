...
class Waist_Coat(Basic_Bodice):
	...
	def __init__(self, pname="M44G", gender='m', style='Gilewska', age=12, ease=8, wc_style = 'Classical', overlap=False):

		Basic_Bodice.__init__(self,pname, gender, style, age, ease, hip=False)
	...
	def cal_wc(self):
		...
		if self.wc_style == 'Classical':
			...
			CPF = BB['SlF1'] + [2,0]
			l_emmanchure_dev, sleeve_front_points  =   self.pistolet( [BB['ShF1'],CPF, BB['SlF1']], 2, tot=True)

			CPB = BB['SlB1'] + [-2,0]
			l_emmanchure_dos, sleeve_back_points  =   self.pistolet( [BB['ShB1'],CPB, BB['SlB1']], 2, tot=True)

			#collars
			BB['HB'] = BB['HB'] + [0,-2]
			collar_CP = BB['HB'] + [1,0]

			l_collar, collar_back_points = self.pistolet([BB['HB'],collar_CP,BB['CB1']], 2, tot=True)
